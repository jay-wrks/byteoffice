...
function showHelp(){ showModal(`<div class="modal-heading"><span>JAVA HANDBOOK</span><h2>Programming Byte</h2></div><div class="help-copy"><p>You write a normal <b>Program.java</b>. ByteOffice owns <code>main()</code>, creates one <code>ByteBot</code>, and calls <code>program(ByteBot bot)</code>.</p><p><b>Java is real:</b> use variables, fields, methods, helper classes, loops, conditions, arrays, collections, recursion and the Java standard library. The source is compiled and executed inside the browser with Java 8.</p><p><b>The physical restriction is ByteBot:</b> <code>bot.take()</code>, <code>bot.send()</code>, <code>bot.copyTo(slot)</code>, <code>bot.copyFrom(slot)</code>, <code>bot.place(slot)</code>, <code>bot.pick(slot)</code>, <code>bot.add(slot)</code> and <code>bot.subtract(slot)</code> never return box values. There is no value getter or peek API.</p><p>Use <code>bot.hasNext()</code>, <code>bot.isZero()</code>, <code>bot.isNegative()</code>, <code>bot.isHolding()</code>, <code>bot.memorySize()</code> and <code>bot.isEmpty(slot)</code> for safe control information.</p><p><b>RUN / Ctrl+Enter</b> continues Java execution. The <b>STEP</b> button releases exactly one physical ByteBot action. The <b>Pause</b> button or <b>Esc</b> stops at the next ByteBot action boundary. <b>Test ×2</b> compiles once and checks both examples.</p><p>Each level still owns its physical floor-memory size. If a level has two slots, calls using slot 2 or higher fail even though your Java program itself remains unrestricted.</p></div>`); }
...
function closeModal(immediate=false){
  ...
  els.modal.classList.remove('java-compile-error-modal','java-api-modal','star-reveal-modal');
