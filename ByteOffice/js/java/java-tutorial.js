(function(){
  ...
  const guides={
    ...
    3:[
      ...
      {target:'#byteMonaco',kicker:'STEP 1 · COMPLETE PROGRAM',title:'Keep handling pairs',body:'Use this complete program to reverse every pair. Byte stores the first box, sends the second box, then sends the stored first box.',code:'while (bot.hasNext()) {\n  bot.take();\n  bot.place(0);\n\n  bot.take();\n  bot.send();\n\n  bot.pick(0);\n  bot.send();\n}',condition:source=>/\bwhile\s*\(\s*bot\s*\.\s*hasNext\s*\(\s*\)\s*\)/.test(source),waiting:'Add the complete pair-swap program in Program.java.'},
      {target:'#byteMonaco',kicker:'STEP 2 · SAVE THE FIRST BOX',title:'Place it in Slot A',body:'After the first <code>bot.take()</code>, place the box in floor memory. <code>place(0)</code> moves it from Byte’s hands into Slot A.',code:'bot.place(0);',condition:source=>/\bbot\s*\.\s*place\s*\(\s*0\s*\)\s*;/.test(source),waiting:'Add bot.place(0); after Byte takes the first box.'},
